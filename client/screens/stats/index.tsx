...
export default function StatsScreen() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      // 并发获取两种统计数据
      const [workerRes, projectRes] = await Promise.all([
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/stats/worker/${year}/${month}`),
        fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/stats/project/${year}/${month}`),
      ]);

      const workerData: StatsResponse = await workerRes.json();
      const projectData: StatsResponse = await projectRes.json();

      setWorkerStats(workerData.stats || []);
      setProjectStats(projectData.stats || []);
    } catch (error) {
